...
from pc_agent_remote.utils import print_with_color
    ...
    def get_service(name: str, model_name: str = None) -> "BaseService":
        ...
        if service_name:
            if name in ["aoai", "azure_ad", "operator"]:
                module = import_module(".openai", package="pc_agent_remote.llm")
                ...
                if custom_service_name:
                    module = import_module("." + custom_model, package="pc_agent_remote.llm")
                    service_name = custom_service_name
                else:
                    raise ValueError(f"Custom model {custom_model} not supported")
            else:
                module = import_module("." + name.lower(), package="pc_agent_remote.llm")
