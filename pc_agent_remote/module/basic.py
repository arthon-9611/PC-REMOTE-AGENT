...
"""
This module contains the basic classes of Round and Session for the PC-AGENT-REMOTE system.

A round of a session in PC-AGENT-REMOTE manages a single user request and consists of multiple steps.

A session may consists of multiple rounds of interactions.

The session is the core class of PC-AGENT-REMOTE. It manages the state transition and handles the different states, using the state pattern.
...
from pc_agent_remote import utils
from pc_agent_remote.agents.agent.app_agent import AppAgent
from pc_agent_remote.agents.agent.basic import BasicAgent
from pc_agent_remote.agents.agent.evaluation_agent import EvaluationAgent
from pc_agent_remote.agents.agent.host_agent import AgentFactory, HostAgent
from pc_agent_remote.agents.states.basic import AgentState, AgentStatus
from pc_agent_remote.automator.ui_control import ui_tree
from pc_agent_remote.automator.ui_control.screenshot import PhotographerFacade
from pc_agent_remote.config.config import Config
from pc_agent_remote.experience.summarizer import ExperienceSummarizer
from pc_agent_remote.module.context import Context, ContextNames
from pc_agent_remote.trajectory.parser import Trajectory
...
class BaseRound(ABC):
    """
    A round of a session in PC-AGENT-REMOTE.
...
class BaseSession(ABC):
    """
    A basic session in PC-AGENT-REMOTE. A session consists of multiple rounds of interactions and conversations.
    """
